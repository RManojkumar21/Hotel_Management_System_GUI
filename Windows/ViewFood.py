...
def viewfood():
    ...
    if c==1:
        ...
        Label(root9,text="FOOD STOCKS",bg="light Grey",fg="Black",font=("mono",20,"italic"),width=14).pack()
        Label(root9,text="",bg="tomato",height=3).pack()
        #quit
        Button(root9,text="QUIT",fg="black",font=("times new roman",9,"bold"),width=10,height=2,command=destroy).place(x=455,y=680)
        #container
        Lf=Frame(root9,bg="ivory2",width=800,height=530)
        Lf.pack()
    
        Label(Lf, text="%-92s%-86s%-50s"%('STOCKNAME', 'QUANTITY', 'TOTALCOST'),bg='ivory2',fg='black').place(x=40,y=20)

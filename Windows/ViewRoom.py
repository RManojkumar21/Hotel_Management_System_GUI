...
def viewroom():
    ...
    if c==1:
        ...
        Label(root10,text="CUSTOMER & ROOM DETAILS",bg="light Grey",fg="Black",font=("ubuntu",20,"italic"),width=26).pack()
        Label(root10,text="",bg="sandybrown",height=3).pack()
        #quit
        Button(root10,text="QUIT",fg="black",font=("times new roman",9,"bold"),width=10,height=2,command=destroy).place(x=450,y=680)
        #container
        Lf=Frame(root10,bg="light goldenrod",width=850,height=530)
        Lf.pack()
        n=70
        Label(Lf, text="%-25s%-16s%-23s%-32s%-30s%-24s%-30s"%('NAME', 'PHONENO', 'ROOMNO', 'MEALS', 'ROOMTYPE', 'CHECKIN', 'CHECKOUT'),bg='light goldenrod',fg='black').place(x=65,y=20)
        ...
        if results:
            ...
            con.commit()
            for i in cur:
                Label(Lf,text="%-23s%-28s%-25s"%(i[0],i[1],i[2]),bg='light goldenrod',fg='black').place(x=160,y=a)
                a+=30
            b=70
            cur.execute(view2)
            con.commit()
            for i in cur:
                Label(Lf,text="%-20s"%(i[0]),bg='light goldenrod',fg='black').place(x=470,y=b)
                b+=30
            c=70
            cur.execute(view3)
            con.commit()
            for i in cur:
                Label(Lf,text="%-27s%-20s"%(i[0],i[1]),bg='light goldenrod',fg='black').place(x=605,y=c)
                c+=30

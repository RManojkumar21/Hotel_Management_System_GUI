...
def viewEmployee():
    ...
    if c==1:   
        ...
        Label(root8,text="EMPLOYEE RECORD",bg="light Grey",fg="Black",font=("ubuntu",20,"italic"),width=18).pack()
        Label(root8,text="",bg="gray21",height=3).pack()
        #Quit
        Button(root8,text="QUIT",fg="black",font=("times new roman",9,"bold"),width=10,height=2,command=destroy).place(x=660,y=680)
        #container
        Lf=Frame(root8,bg="ivory2",width=1300,height=530)
        Lf.pack()
        Label(Lf, text="%-25s%-10s%-20s%-10s%-25s%-40s%-45s%-20s%-18s%-25s%-20s%-35s%-15s%-5s"%('NAME', 'AGE', 'DOB', 'GENDER', 'CONTACT', 'EMAIL', 'ADDRESS', 'CITY', 'STATE', 'DEPARTMENT', 'STATUS', 'IDPROOF', 'TYPE', 'SALARY'),bg='ivory2',fg='black').place(x=10,y=20)
        ...
        if results:
            ...
            con.commit()
            for i in cur:
                Label(Lf,text="%-8s"%(i[0]),bg='ivory2',fg='black').place(x=680,y=f)
                f+=30
            g=80
            cur.execute(view7)
            con.commit()
